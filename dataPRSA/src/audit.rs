...
fn print_audit_summary(report: &AuditReport) {
    ...
    if red > 0 {
        println!("  {} Nulls    : {} kolom bermasalah", "🔴".red(), red);
    }
    if yellow > 0 {
        println!("  {} Nulls    : {} kolom perlu perhatian", "🟡".yellow(), yellow);
    }
    if red == 0 && yellow == 0 {
        println!("  {} Nulls    : semua kolom relatif bersih", "✅".green());
    }

    if outlier_red > 0 {
        println!(
            "  {} Outliers : {} kolom dengan outlier > 5%",
            "🔴".red(),
            outlier_red
        );
    } else {
        println!("  {} Outliers : tidak ada outlier signifikan", "✅".green());
    }
}
