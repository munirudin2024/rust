...
pub fn build_station_summary(
    ...
    let payment_stats = build_payment_stats(clean_df);
    let city_revenue_stats = build_city_revenue_stats(clean_df);
...
fn cleanup_legacy_root_outputs(output_root: &Path) -> Result<()> {
  let runs_dir = output_root.join("runs");
  if runs_dir.exists() {
    std::fs::remove_dir_all(&runs_dir).context("failed to remove deprecated output/runs")?;
  }

  let latest_dir = output_root.join("latest");
  if latest_dir.exists() {
    std::fs::remove_dir_all(&latest_dir)
      .context("failed to remove deprecated output/latest")?;
  }

    for file_name in ["report.html", "report_data.json"] {
        let file_path = output_root.join(file_name);
        if file_path.exists() {
            std::fs::remove_file(&file_path)
                .with_context(|| format!("failed to remove legacy {}", file_name))?;
        }
    }
...
<head>
  ...
  <title>Index Laporan Data</title>
  <style>
    :root {{
      --bg: #f8f4ec;
      --card: #ffffff;
    ...
    * {{ box-sizing: border-box; }}
    ...
    .hero {{
      background: linear-gradient(120deg, #0f766e, #14b8a6 50%, #f97316 120%);
      color: #fff;
      padding: 24px;
      border-radius: 20px;
...
<body>
  <div class="wrap">
    ...
    <div class="grid">
      ...
      <section class="card">
        <h2>Data Bersih CSV</h2>
        <div class="meta">Setiap file CSV hasil cleaning disimpan ke folder output/csv dengan nama file asal.</div>
        <a class="btn" href="csv/">Buka folder csv</a>
      </section>
    </div>
...
<body>
      ...
      <main>
        ...
        <section class="section">
          ...
          <div id="tab_wilayah" class="tab-panel">
            ...
            <div id="payment_chart" class="chart"></div>
            <div id="city_spend_chart" class="chart"></div>
          </div>
  ...
  <script>
    ...
    function renderAll() {{
      ...
      renderPaymentChart(filteredStations);
      renderCitySpendChart(filteredStations);
    ...
    function renderPaymentChart(stationsData) {{
      const all = (stationsData.length > 0 ? stationsData : reportData);
      const agg = new Map();
      all.forEach((s) => {{
        (s.payment_stats || []).forEach((p) => {{
          agg.set(p.method, (agg.get(p.method) || 0) + (p.count || 0));
        }});
      }});
      ...
      Plotly.newPlot('payment_chart', [{{
        ...
        title: 'Metode Pembayaran Paling Populer',
        yaxis: {{ title: 'Jumlah Transaksi' }}
      }}, {{ responsive: true }});
    }}

    function renderCitySpendChart(stationsData) {{
      const all = (stationsData.length > 0 ? stationsData : reportData);
      const agg = new Map();
      all.forEach((s) => {{
        (s.city_revenue_stats || []).forEach((c) => {{
          agg.set(c.city, (agg.get(c.city) || 0) + (c.total_revenue || 0));
        }});
      }});

      const items = Array.from(agg.entries()).sort((a, b) => b[1] - a[1]).slice(0, 10);
      const labels = items.map((x) => x[0]);
      const values = items.map((x) => x[1]);

      Plotly.newPlot('city_spend_chart', [{{
        type: 'bar',
        x: labels,
        y: values,
        marker: {{ color: '#16a34a' }}
      }}], {{
        title: 'Kota dengan Belanja Terbesar (Top 10)',
...
fn build_payment_stats(df: &DataFrame) -> Vec<PaymentStat> {
    let payment_col = find_column_name(
        df,
        &[
            "Metode_Pembayaran",
            "metode_pembayaran",
    ...
    let Some(col_name) = payment_col else {
        return Vec::new();
    };

    let Ok(s) = df.column(&col_name) else {
        return Vec::new();
    };
    ...
    for i in 0..s.len() {
        if let Ok(v) = s.get(i) {
            ...
            let key = anyvalue_to_plain_string(v).trim().to_ascii_uppercase();
            if !key.is_empty() {
    ...
    let mut rows: Vec<PaymentStat> = counts
        .into_iter()
        .map(|(method, count)| PaymentStat { method, count })
        .collect();
    rows.sort_by(|a, b| b.count.cmp(&a.count));
    rows
}

fn build_city_revenue_stats(df: &DataFrame) -> Vec<CityRevenueStat> {
    let city_col = find_column_name(df, &["Kota_Tujuan", "kota_tujuan", "kota", "city"]);
    let revenue_col = find_column_name(df, &["revenue_per_transaction"]);
    let (Some(city_col), Some(revenue_col)) = (city_col, revenue_col) else {
        return Vec::new();
    };

    let Ok(city_s) = df.column(&city_col) else {
        return Vec::new();
    };
    let Ok(rev_s) = df.column(&revenue_col) else {
        return Vec::new();
    };
    ...
    for i in 0..df.height() {
        let city = city_s
            ...
            .map(|v| anyvalue_to_plain_string(v).trim().to_ascii_uppercase());
        let revenue = get_f64_at(rev_s, i);
        if let (Some(c), Some(r)) = (city, revenue) {
            *sums.entry(c).or_insert(0.0) += r;
        }
    }

    let mut rows: Vec<CityRevenueStat> = sums
        .into_iter()
        .map(|(city, total_revenue)| CityRevenueStat {
            city,
            total_revenue,
        })
        .collect();
